...
const CopyButton = ({ id }: { id: string }) => {
    ...
    return (
        <div onClick={handleCopy} className='p-2 hover:scale-105 cursor-pointer hover:bg-zinc-700 rounded-md relative'>
            <IoCheckmarkOutline className={cn("cursor-pointer transition-all w-5 h-5 text-green-500", onDone ? "scale-100" : "scale-0")}
                onTransitionEnd={() => {
                    setTimeout(() => {
                        setCopy(false);
                        setDone(false);
            ...
            <div className='h-full w-full absolute top-0 left-0 flex items-center justify-center '>
                <BsCopy className={cn("transition-all", onCopy ? "scale-0" : "scale-100")} onTransitionEnd={() => {
                    if (onCopy) {
                        setDone(true)
                    }
                }} />
